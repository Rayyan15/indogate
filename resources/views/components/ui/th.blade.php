...
<th scope="col" {{ $attributes->merge(['class' => 'px-4 py-3 text-center first:ps-5 last:pe-5']) }}>
    @if($sortable && $field)
        <button type="button" wire:click="sortBy('{{ $field }}')" class="inline-flex items-center justify-center gap-1 {{ $sortField === $field ? 'text-red-600' : 'hover:text-neutral-900' }}">
            {{ $slot }}
            @if($sortField === $field)
