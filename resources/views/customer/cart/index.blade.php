            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-12">
                <main class="space-y-4 lg:col-span-8">
                    <div class="divide-y divide-neutral-200 rounded border border-neutral-200 bg-neutral-0">
                        @foreach($cart as $index => $item)
                            ...
                            <x-ui.segment :index="$loop->iteration" :kicker="class_basename($item['bookable_type']) . ' ' . __('customer.cart.service')" :title="$item['name']" :meta="$item['quantity'] . ' × IDR ' . number_format($item['price'])" :amount="'IDR ' . number_format($subtotal)">
                                <x-slot name="actions">
                ...
                <aside class="lg:col-span-4">
                    <x-ui.folio :eyebrow="__('customer.cart.summary_eyebrow')" :title="__('customer.cart.summary_title')"
                        :rows="[[__('customer.cart.subtotal'), 'IDR ' . number_format($total)], [__('customer.cart.concierge_fee'), __('customer.cart.complimentary')], [__('customer.cart.taxes'), __('customer.cart.calculated_at_checkout')]]"
                        :total-label="__('customer.cart.estimated_total')" :total="'IDR ' . number_format($total)">
